...
namespace vista
{
		...
		ID3D12CommandList* const* commandLists)
	{
		...
		if (!rawCommandLists.empty())
		{
			commandQueue->ExecuteCommandLists(
				static_cast<UINT>(rawCommandLists.size()),
				rawCommandLists.data());
		...
		std::vector<std::vector<D3D12_RESOURCE_BARRIER>>& fixingBarriersPerList)
	{
		...
		for (Uint i = 0; i < numCommandLists; ++i)
		{
			...
			for (auto& [resource, state] : resourceStates)
			{
				D3D12_RESOURCE_STATES stateBeforeList = lastKnownStates.at(resource);
				...
				if (clStateIt != state.commandListStates.end())
				{
					...
					if (stateBeforeList != D3D12_RESOURCE_STATE_COMMON)
					{
				...
				{
					lastKnownStates[resource] = stateBeforeList;
				}
			}
		}
		...
		for (auto& [resource, state] : resourceStates)
		{
			state.globalState = lastKnownStates.at(resource);
		}
	}
