...
namespace vista
{
	DxilModuleWrapper::DxilModuleWrapper(void const* shaderBytecode, Uint64 bytecodeSize)
	{
		...
		if (pContainer != nullptr)
		{
			...
			hlsl::DxilPartIterator it = std::find_if(begin(pContainer), end(pContainer), hlsl::DxilPartIsType(hlsl::DFCC_DXIL));
			VISTA_ASSERT_MSG(it != end(pContainer), "No DXIL part found in shader container!");
	...
	DxilModuleWrapper::~DxilModuleWrapper() = default;

	hlsl::DxilModule& DxilModuleWrapper::GetDxilModule()
	{
		return *dxilModule;
	}
